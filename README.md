...
Text and image files will also have their contents read and stored in another table called `contents`.  This contents table has the following schema:

field | description
----- | ------------
digest | SHA256 hash of the file's contents.
data | Binary-read data from the file.
created_at | UTC date and time when the record was inserted into the DB.
